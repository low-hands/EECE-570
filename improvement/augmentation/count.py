...
if __name__=="__main__":
    ...
    for cur_json in tqdm(src_list):
        ...
        generate_all_cnt_prob=1#np.random.uniform()
        if generate_all_cnt_prob>0.5:
            distrub_bbox_question=f"<image>仔细检查图像，输出图像中['person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic light', 'fire hydrant', 'stop sign', 'parking meter', 'bench', 'bird', 'cat', 'dog', 'horse', 'sheep', 'cow', 'elephant', 'bear', 'zebra', 'giraffe', 'backpack', 'umbrella', 'handbag', 'tie', 'suitcase', 'frisbee', 'skis', 'snowboard', 'sports ball', 'kite', 'baseball bat', 'baseball glove', 'skateboard', 'surfboard', 'tennis racket', 'bottle', 'wine glass', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'banana', 'apple', 'sandwich', 'orange', 'broccoli', 'carrot', 'hot dog', 'pizza', 'donut', 'cake', 'chair', 'couch', 'potted plant', 'bed', 'dining table', 'toilet', 'tv', 'laptop', 'mouse', 'remote', 'keyboard', 'cell phone', 'microwave', 'oven', 'toaster', 'sink', 'refrigerator', 'book', 'clock', 'vase', 'scissors', 'teddy bear', 'hair drier', 'toothbrush']类别的数量"
            generate_all_cnt_string=""
            for name in coco_list:
                if name in bboxs_dict:
                    cnt = len(bboxs_dict[name])
                    generate_all_cnt_string+=f"{name}:{cnt}\n"
                else:
                    generate_all_cnt_string+=f"{name}:0\n"
            save_list.append(generate_qwenvl(image_path,distrub_bbox_question,generate_all_cnt_string))
        #随机选择一个框作为初始框
                    
        #print(cnt_last)
    save_path=r"D:\UBC\eece570\data\count.json"
