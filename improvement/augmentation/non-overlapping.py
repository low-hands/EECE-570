...
def generate_unvalid_ankor(choose_bbox,bboxs):
    ...
    cnt=0
    unvalid_ankor=[]
    while iou>0.05 and cnt<10000:
        cnt+=1
        x1_random = random.randint(0, 1000-w)
        y1_random = random.randint(0, 1000-h)
...
def generate_all_overlap_ankor(bbox,cur_need_iou,bboxs):
    ...
    iou=-1
    cur_iter_cnt=0
    while not (iou>cur_need_iou and iou<cur_need_iou+0.1) and cur_iter_cnt<100000:
...
if __name__=="__main__":
    ...
    for cur_json in tqdm(src_list):
        ...
        for choose_bbox_index in range(bboxs_cnt):
            ...
            generate_bbox=generate_unvalid_ankor(choose_bbox,all_boxes)
            if generate_bbox is None:
                print("gga")
                continue
            location_str=f"<|box_start|>({generate_bbox[0]},{generate_bbox[1]}),({generate_bbox[2]},{generate_bbox[3]})<|box_end|>"
            generate_unvalid_question=f"请观察图像中的{location_str}区域，是否有任意一个['person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic light', 'fire hydrant', 'stop sign', 'parking meter', 'bench', 'bird', 'cat', 'dog', 'horse', 'sheep', 'cow', 'elephant', 'bear', 'zebra', 'giraffe', 'backpack', 'umbrella', 'handbag', 'tie', 'suitcase', 'frisbee', 'skis', 'snowboard', 'sports ball', 'kite', 'baseball bat', 'baseball glove', 'skateboard', 'surfboard', 'tennis racket', 'bottle', 'wine glass', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'banana', 'apple', 'sandwich', 'orange', 'broccoli', 'carrot', 'hot dog', 'pizza', 'donut', 'cake', 'chair', 'couch', 'potted plant', 'bed', 'dining table', 'toilet', 'tv', 'laptop', 'mouse', 'remote', 'keyboard', 'cell phone', 'microwave', 'oven', 'toaster', 'sink', 'refrigerator', 'book', 'clock', 'vase', 'scissors', 'teddy bear', 'hair drier', 'toothbrush']被框中？若有，请描述框中的类别和框中的程度。"#f"仔细检查图像，判断图像中{location_str}区域是否完全框中了任意一个真实人脸，如果完全框中，请输出“完全框中”，如果框中部分人脸，请输出“部分框中”以及框中人脸的程度，如果没有框中，请输出“没有框中”"
            generate_unvalid_answer=f"在{location_str}区域中，没有任何类别被框中"
            unvalid_ankor_dict=generate_qwenvl(image_path,generate_unvalid_question,generate_unvalid_answer)
            save_list.append(unvalid_ankor_dict)
            save_cnt+=1
            cnt_last+=1
        ...
        #print(cnt_last)
    save_path=r"D:\UBC\eece570\data\non-overlapping.json"
