...
if __name__=="__main__":
    cnt_last=0
    src_qwenvl_path="D:\UBC\eece570\data\coco0408_train_prompt.json"
    ...
    for cur_json in tqdm(src_list):
        ...
        for choose_bbox_index in range(bboxs_cnt):
            ...
            small_distrub_prob=np.random.uniform()   
            if small_distrub_prob>0.3:#0.3
                distrub_bbox=ankor_small_distrub(choose_bbox,iou_thread=0.9,number=10)
                small_cnt=0
                for other_bbox in all_boxes:
                    cur_iou=bbox_iou(distrub_bbox,other_bbox[1])
                    if cur_iou>0.05:
                        small_cnt+=1
                if cnt_1>=3:
                    break
                if small_cnt==1:     
                    ...
                    distrub_bbox_question=f"请观察图像中的{location_str}区域，是否有['person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic light', 'fire hydrant', 'stop sign', 'parking meter', 'bench', 'bird', 'cat', 'dog', 'horse', 'sheep', 'cow', 'elephant', 'bear', 'zebra', 'giraffe', 'backpack', 'umbrella', 'handbag', 'tie', 'suitcase', 'frisbee', 'skis', 'snowboard', 'sports ball', 'kite', 'baseball bat', 'baseball glove', 'skateboard', 'surfboard', 'tennis racket', 'bottle', 'wine glass', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'banana', 'apple', 'sandwich', 'orange', 'broccoli', 'carrot', 'hot dog', 'pizza', 'donut', 'cake', 'chair', 'couch', 'potted plant', 'bed', 'dining table', 'toilet', 'tv', 'laptop', 'mouse', 'remote', 'keyboard', 'cell phone', 'microwave', 'oven', 'toaster', 'sink', 'refrigerator', 'book', 'clock', 'vase', 'scissors', 'teddy bear', 'hair drier', 'toothbrush']被框中？若有，请描述框中的类别和框中的程度。"
                    #distrub_bbox_question=f"请观察图像中的{location_str}区域，请描述框中的类别和框中的程度。"
                    distrub_bbox_answer=f"在{location_str}区域中，框中了{classes_name}类别，框中程度为“完全框中”"
                    distrub_bbox_dict=generate_qwenvl(image_path,distrub_bbox_question,distrub_bbox_answer)
                    save_list.append(distrub_bbox_dict)
                    save_cnt+=1 

        #print(cnt_last)
    save_path="D:\UBC\eece570\data\full-overlapped.json"
